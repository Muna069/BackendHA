...
router.post("/add", uploadExercise.fields([
  ...
  try {
    ...
    const newExercise = new Exercise({
      ...
      workoutGif: req.files["workoutGif"][0].path, // Cloudinary URL
    });
...
router.get("/all", async (req, res) => {
  ...
  router.get("/count", async (req, res) => {
    try {
      const count = await Exercise.countDocuments(); // Counts the total number of documents in the Exercise collection
      res.json({ count });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  ...
  // Schedule a cron job to reset lastCompleted at midnight
